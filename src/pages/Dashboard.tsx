         ...
         <div className="flex items-center gap-3 bg-zen-dark px-4 py-2 rounded-lg border border-gray-800">
            ...
            <div className="relative">
              <select 
                 ...
                 className="appearance-none bg-transparent text-xl font-mono font-bold text-white outline-none cursor-pointer pr-8 border-b border-dashed border-gray-600 hover:border-zen-accent hover:text-zen-accent transition-colors"
                 disabled={loading}
              >
                {Array.from({ length: maxCycle + 1 }, (_, i) => maxCycle - i).map((num) => (
                  <option key={num} value={num} className="bg-zen-card text-base">{num}</option>
                ))}
              </select>
