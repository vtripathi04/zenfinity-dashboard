  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const pdf = new jsPDF({
        ...
        unit: 'mm',
        format: [scrollWidth * pxToMm, scrollHeight * pxToMm],
      });
      pdf.addImage(imgData, 'PNG', 0, 0, scrollWidth * pxToMm, scrollHeight * pxToMm);
      pdf.save(`Analytics_Report_${selectedImei}_C${cycleNumber}.pdf`);
    } catch (err) { console.error("PDF Failed", err); alert("Failed to generate PDF"); } 
    ...
    <Layout>
      ...
      <div className="bg-zen-card border border-gray-800 rounded-xl p-3 mb-8 flex flex-col md:flex-row items-center justify-center shadow-lg gap-4 md:gap-6">
         ...
         <button 
            ...
            className="p-2 hover:bg-zen-cardHover rounded-lg text-zen-accent disabled:opacity-30 transition-colors" 
         ...
         <div className="flex items-center bg-zen-dark rounded-lg border border-gray-700 px-4 py-2 shadow-inner group relative w-48" ref={dropdownRef}>
            <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold mr-3 select-none">Cycle</span>
            
            <button 
              onClick={() => setIsCycleDropdownOpen(!isCycleDropdownOpen)}
              className="flex-1 flex items-center justify-between text-white font-mono font-bold text-lg outline-none cursor-pointer"
              disabled={loading}
            >
              {cycleNumber}
              <ChevronDown size={16} className={`text-zen-accent transition-transform ${isCycleDropdownOpen ? 'rotate-180' : ''}`} />
            </button>

            {/* Custom Dropdown Menu */}
            {isCycleDropdownOpen && (
              <div className="absolute top-full left-0 mt-2 w-full bg-zen-dark border border-gray-700 rounded-lg shadow-2xl z-50 max-h-60 overflow-y-auto custom-scrollbar">
                {Array.from({ length: maxCycle + 1 }, (_, i) => maxCycle - i).map((num) => (
                  <div 
                    ...
                    className={`px-4 py-2 cursor-pointer transition-colors text-sm font-mono ${
                      num === cycleNumber ? 'bg-zen-accent text-white' : 'text-gray-300 hover:bg-zen-cardHover hover:text-white'
                    }`}
                  >
                    {num}
                  </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
              ...
              <MetricCard label="Efficiency" value={(() => {
                ...
                })()} color="text-blue-400" icon={TrendingUp} />
            </div>
                   ...
                   {data.alert_details?.warnings?.length === 0 && data.alert_details?.protections?.length === 0 ? (
                     ...
                     <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar">
                       {data.alert_details?.warnings?.map((w, i) => (
                         <div key={`w-${i}`} className="flex items-center gap-2 text-yellow-500 bg-yellow-900/10 p-2 rounded text-sm"><AlertTriangle size={14} className="shrink-0" /><span>{w}</span></div>
                       ))}
                       {data.alert_details?.protections?.map((p, i) => (
                         <div key={`p-${i}`} className="flex items-center gap-2 text-red-400 bg-red-900/10 p-2 rounded text-sm"><AlertCircle size={14} className="shrink-0" /><span>{p}</span></div>
                       ))}
                     </div>
                   )}
                 </div>
